# -*- coding: utf-8 -*-
"""
Koib-V-4.2 — Модуль парсинга документов
==========================================
        ...
        try:
            ...
            for img_info in page.get_images(full=True):
                try:
                    ...
                    # Подпись и контекст: определяем положение изображения
                    # через get_image_rects(), а не через img_info[1:5]
                    # (img_info содержит xref, smask, width, height, bpc, ...
                    #  — bounding box там отсутствует).
                    try:
                        rects = page.get_image_rects(xref)
                        raw_rect = rects[0] if rects else fitz.Rect(0, 0, 0, 0)
                    except Exception:
                        raw_rect = fitz.Rect(0, 0, 0, 0)
